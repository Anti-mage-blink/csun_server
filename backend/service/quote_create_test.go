...
// MockQuoteCreateRepository is a mock implementation of repository.QuoteCreateRepository
type MockQuoteCreateRepository struct {
...
func (m *MockQuoteCreateRepository) GetMaxQuoteCodeByPrefix(ctx context.Context, prefix string) (*string, error) {
	return m.GetMaxQuoteCodeByPrefixFn(ctx, prefix)
}

func (m *MockQuoteCreateRepository) GetAllCustomers(ctx context.Context) ([]*general.Customer, error) {
	return m.GetAllCustomersFn(ctx)
}

func (m *MockQuoteCreateRepository) GetAllProductCategories(ctx context.Context) ([]*product.ProductCategory, error) {
	return m.GetAllProductCategoriesFn(ctx)
}

func (m *MockQuoteCreateRepository) GetAllProductNames(ctx context.Context) ([]*product.ProductName, error) {
	return m.GetAllProductNamesFn(ctx)
}

func (m *MockQuoteCreateRepository) GetAllProductSpecs(ctx context.Context) ([]*product.ProductSpec, error) {
	return m.GetAllProductSpecsFn(ctx)
}

func (m *MockQuoteCreateRepository) GetAllPriceCatalogs(ctx context.Context) ([]*product.PriceCatalog, error) {
	return m.GetAllPriceCatalogsFn(ctx)
}

// newDefaultMockRepo 返回一个所有查询都返回空切片成功的 mock，便于聚焦测试核心逻辑
func newDefaultMockRepo() *MockQuoteCreateRepository {
	return &MockQuoteCreateRepository{
		GetMaxQuoteCodeByPrefixFn: func(ctx context.Context, prefix string) (*string, error) {
			return nil, nil
		},
...
func TestQuoteCreateService_PrepareQuoteCreate(t *testing.T) {
	ctx := context.Background()

	t.Run("No existing quote code - returns 001", func(t *testing.T) {
		mockRepo := newDefaultMockRepo()
		svc := NewQuoteCreateService(mockRepo)

		data, err := svc.PrepareQuoteCreate(ctx)
		if err != nil {
	...
	t.Run("Repository error propagates", func(t *testing.T) {
		...
		_, err := svc.PrepareQuoteCreate(ctx)
