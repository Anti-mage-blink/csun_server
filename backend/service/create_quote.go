...
type createQuoteService struct {
	createQuoteRepo repository.CreateQuoteRepository
}

func NewCreateQuoteService(createQuoteRepo repository.CreateQuoteRepository) CreateQuoteService {
	return &createQuoteService{createQuoteRepo: createQuoteRepo}
}
...
func (s *createQuoteService) PrepareCreateQuote(ctx context.Context) (*CreateQuoteData, error) {
	...
	maxCode, err := s.createQuoteRepo.GetMaxQuoteCodeByPrefix(ctx, prefix)
	if err != nil {
		return nil, err
	}
	...
	// 2. 获取全量数据表，方便前端使用
	customers, err := s.createQuoteRepo.GetAllCustomers(ctx)
	if err != nil {
		return nil, err
	}
	productCategories, err := s.createQuoteRepo.GetAllProductCategories(ctx)
	if err != nil {
		return nil, err
	}
	productNames, err := s.createQuoteRepo.GetAllProductNames(ctx)
	if err != nil {
		return nil, err
	}
	productSpecs, err := s.createQuoteRepo.GetAllProductSpecs(ctx)
	if err != nil {
		return nil, err
	}
	priceCatalogs, err := s.createQuoteRepo.GetAllPriceCatalogs(ctx)
	if err != nil {
		return nil, err
	}
